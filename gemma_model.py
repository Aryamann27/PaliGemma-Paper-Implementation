...
class PaliGemmaForConditionalGeneration(nn.Module):
    def __init__(self, config: PaliGemmaConfig):
        ...
        # linear layer that reshapes the image embeddings to the size of text embeddings or tokens 
        self.multi_modal_projector = PaliGemmaForConditionalGeneration(config)
        self.vocab_size = config.vocab_size
    ...
    # tie_weights is a technique where one layer shares the parameters of another layer. this technique helps reduce the overall number of parameters in the model
    def tie_weights(self):
    ...
    def _merge_input_ids_with_image_features(
        ...
        # We need to exppnad the masks to the embedding dimension otherwise we can't use them in torch.where
        ...
        final_embedding = torch.where(pad_mask_expanded, torch.zeros_line(final_embedding), final_embedding) 
        ...
        if kv_cache is not None and kv_cache.num_items()>0:
            ...
            # Create a position_id based on the size of the attention_mask
